...
let iframe = new Iframe(window.prettyblocks.routes.front_office, 1, 1)
...
emitter.on('changeUrl', (shop, custom_url = null) => {
  ...
  // Si `shop` est null, utilisez les valeurs par défaut
  iframe.setIdLang(shop?.id_lang || iframe.id_lang);
  iframe.setIdShop(shop?.id_shop || iframe.id_shop);
